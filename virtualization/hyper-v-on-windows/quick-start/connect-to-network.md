...
Hyper-V поддерживает три типа виртуальных коммутаторов: внешние, внутренние и частные. Создайте внешний коммутатор, чтобы предоставить общий доступ к сети вашего компьютера с помощью виртуальных машин, на которых они запущены.
...
Если вы используете Windows 10 Hyper-V на ноутбуке и часто переключаться между беспроводной сетью и проводной сетью, вам может понадобиться создать виртуальный коммутатор как для плат Ethernet, так и для беспроводных сетей.  В зависимости от того, как ноутбук подключен к сети, вы можете изменить параметры виртуальных машин между ними. Виртуальные машины не переключаются между проводных и беспроводными компьютерами автоматически. 

>[!IMPORTANT]
>Два варианта переключения не поддерживают внешний vSwitch через карту беспроводной связи, и его следует использовать только в целях тестирования.
