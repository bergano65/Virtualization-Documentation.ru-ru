...
* **Стандартные контрольные точки**: делает снимок состояния виртуальной машины и памяти виртуальной машины на момент инициации контрольной точки. Моментальный снимок не является полной резервной копией и может приводить к проблемам с согласованностью данных в системах, которые реплицируют данные между различными узлами, например Active Directory.  До Windows10 Hyper-V предоставлял только стандартные контрольные точки (прежде называвшиеся моментальными снимками).

* **Производственные контрольные точки**: использует службу теневого копирования тома или файловую систему, зафиксированную на виртуальной машине Linux, для создания резервной копии виртуальной машины, учитывающей данные. Моментальный снимок состояния памяти для виртуальной машины не создается.
...
В определенной точке могут быть созданы много контрольных точек.  Предоставление понятного имени позволяет запомнить сведения о состоянии системы после создания контрольной точки.
...
1. В **диспетчере Hyper-V** выберите виртуальную машину.
2. В разделе **контрольные точки** щелкните правой кнопкой мыши контрольную точку, которую вы хотите удалить, и выберите команду Удалить. Можно также удалить контрольную точку и все последующие контрольные точки. Для этого щелкните правой кнопкой мыши самую раннюю контрольную точку, которую хотите удалить, и выберите элемент ***Удалить поддерево**контрольных точек**.
...
1. Закройте текстовый файл, если он еще открыт, и удалите его с рабочего стола виртуальной машины.
...
1. Войдите в виртуальную машину и создайте текстовый файл. Если вы выполнили Предыдущее упражнение, вы можете использовать существующий текстовый файл.
...
1. Закройте текстовый файл, если он еще открыт, и удалите его с рабочего стола виртуальной машины.
2. Откройте диспетчер Hyper-V, щелкните правой кнопкой мыши рабочую контрольную точку и выберите **Apply (применить**).
3. В диалоговом окне "Применить контрольную точку" нажмите кнопку **Применить**.
