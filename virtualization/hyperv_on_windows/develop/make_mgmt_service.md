# Создание новой службы управления

Начиная с Windows 10, Hyper-V позволяет устанавливать зарегистрированные подключения через сокет между гостевой операционной системой и узлом Hyper-V без использования сетевого подключения. С помощью сокетов Hyper-V службы могут работать независимо от сетевого стека, и все данные остаются в той же физической памяти.
...
* Пакет SDK для Windows: можно загрузить по ссылке на [пакет SDK для Win10](https://dev.windows.com/en-us/downloads/windows-10-sdk), который включает `hvsocket.h`.
...
> ** Совет. ** Чтобы создать код GUID в PowerShell и скопировать его в буфер обмена, используйте команду:
``` PowerShell
