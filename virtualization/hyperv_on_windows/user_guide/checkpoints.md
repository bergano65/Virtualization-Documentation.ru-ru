...
* **Стандартные контрольные точки** — это контрольные точки, при инициировании которых создается моментальный снимок виртуальной машины и состояния ее памяти. Моментальный снимок не является полной резервной копией и может приводить к проблемам согласованности данных в системах, которые реплицируют данные между различными узлами, например Active Directory. До Windows 10 Hyper-V предоставлял только стандартные контрольные точки (прежде называвшиеся моментальными снимками).

* **Рабочие контрольные точки** — контрольные точки, при инициировании которых создается согласованная резервная копия виртуальной машины с помощью службы теневого копирования томов или "заморозки" файловой системы (на виртуальной машине Linux).
...
>   В этом документе используется термин checkpoint (контрольная точка), но в аналогичных командах также может использоваться термин snapshot (моментальный снимок).
...
Создает контрольную точку того типа, который настроен для данной виртуальной машины. Сведения о том, как изменить тип контрольной точки, см. в разделе [Изменение типа контрольной точки](checkpoints.md#changing-the-checkpoint-type-for-a-VM) выше.
...
Имя должно содержать не более 100 знаков, это поле не может быть пустым.
...
По умолчанию файлы конфигурации контрольных точек сохраняются в папке: `%systemroot%\ProgramData\Microsoft\Windows\Hyper-V\Snapshots`.
...
3. См. инструкции по изменению типа контрольной точки на стандартный [здесь](checkpoints.md#changing-the-checkpoint-type-using-hyper-V-manager).
...
1. Войдите в виртуальную машину и создайте текстовый файл. Если вы выполняли предыдущее упражнение, можно использовать существующий текстовый файл.
