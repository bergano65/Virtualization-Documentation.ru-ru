...
![Аналогия кухню](media/box1.png)

Теперь мы возьмем этот контейнер и поместим его в любую квартиру, при этом кухня никак не изменится. Все, что нам нужно только подключить электричество и воду к нему, и затем мы начать готовить (так как у нас есть все устройства, что нам нужно!).

![Аналогия подразделении](media/apartment.png)
...
Контейнеры— это изолированная и переносимая среда выполнения с контролируемыми ресурсами, которая работает на хост-компьютере или виртуальной машине. Приложения или процессы, которые запускаются в контейнере, поставляются вместе со всеми необходимыми компонентами и файлами конфигурации; он предоставил иллюзию, что нет без процессов, выполняющихся вне контейнера.

Узел контейнера предоставляет набор ресурсов, и контейнер будет использовать только их. Контейнер считает, никаких других ресурсов не существует, что он предоставляется и таким образом контейнера не сенсорных ресурсы, которые могут заказчика инициализированы соседнему контейнеру.

Следующие основные понятия будут полезны, так как чем начинать создание и работы с контейнерами Windows.

**Узел контейнера:** Физической или виртуальной компьютера настроенный для работы с контейнерами Windows. На узле контейнера работает один или несколько контейнеров Windows.
...
**Репозиторий контейнера.** При каждом создании образа контейнера этот образ и его зависимости сохраняются в локальном репозитории. Эти образы можно использовать повторно много раз на узле контейнера. Образы контейнеров также можно хранить в открытом или закрытом реестре, например, Docker Hub, чтобы их можно использовать на многих других узлах контейнеров.

![Принципы работы контейнера](media/containerfund.png)
...
Контейнеры Windows делятся на два типа бывают.
...
![Контейнеры Docker](media/docker.png)

Так же, как и любым другим контейнером контейнер Windows Server можно управлять с помощью [Docker](https://www.docker.com).
...
Azure предлагает два оркестратора контейнеров: служба Kubernetes Azure (AKS) и Service Fabric.

[Служба Kubernetes Azure (AKS)](/azure/aks/) упрощает создание и настройка управления кластером виртуальных машин, подготовленных для запуска приложений в контейнерах. Таким образом вы можете применить существующие навыки или воспользоваться огромным пластом информации, представленным в сообществе, чтобы развертывать контейнерные приложения на Microsoft Azure и управлять ими. Благодаря AKS можно воспользоваться преимуществами корпоративного уровня Azure, сохраняя при этом возможность переноса приложений через Kubernetes и формат образов Docker.
