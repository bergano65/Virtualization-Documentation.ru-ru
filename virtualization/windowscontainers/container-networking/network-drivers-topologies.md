...
- **nat**— контейнеры, подключенные к сети, которая была создана с помощью драйвера "nat", будут подключены к *внутреннему* коммутатору Hyper-V и получать IP-адрес из указанного пользователем префикса IP-адреса (``--subnet``). Перенаправление и сопоставление портов из узла контейнера в конечные точки контейнера поддерживается.
  > В обновлении Windows 10 Creators Update теперь реализована поддержка нескольких сетей NAT!
...
- **overlay**— когда подсистема Docker запущена в [режиме мелких объектов](../manage-containers/swarm-mode.md), контейнеры, подключенные к сети наложения, могут связываться с другими контейнерами, подключенными к той же сети между несколькими узлами контейнера. Каждая сеть наложения, созданная в кластере мелких объектов, создается с собственной IP-подсетью, заданной частным IP-префиксом. Драйвер сети наложения использует инкапсуляцию VXLAN. **Можно использовать с Kubernetes при использовании с подходящими плоскостями контроля сети (Flannel или OVN).**
  > Примечание. Убедитесь, что ваша среда соответствует этим *обязательным* [требованиям](https://docs.docker.com/network/overlay/#operations-for-all-overlay-networks) для создания сетей наложения.

  > Примечание. Требуется Windows Server 2016 с [KB4015217](https://support.microsoft.com/en-us/help/4015217/windows-10-update-kb4015217), Windows 10 Creators Update или более поздний выпуск.
...
- **l2bridge**— контейнеры, которые подключены к сети, созданной с использованием драйвера l2bridge, будут расположены в той же IP-подсети, что и узел контейнеров, и подключены к физической сети через *внешний* коммутатор Hyper-V. IP-адреса должны назначаться статически из того же префикса, что и узел контейнера. У всех конечных точек контейнера на узле будет одинаковый с узлом MAC-адрес из-за преобразования адресов второго уровня (перезапись MAC-адресов) во входящем и исходящем трафике.
  > Примечание. При использовании этого режима в сценарии виртуализации (роль узла контейнеров выполняет виртуальная машина), _требуется спуфинг MAC-адресов_.
  
  > Примечание. Требуется Windows Server 2016, Windows 10 Creators Update или более поздний выпуск.

- **l2tunnel**— аналогично l2bridge, однако _этот драйвер должен использоваться только в Microsoft Cloud Stack_. Пакеты, поступающие из контейнера, отправляются на узел виртуализации, на котором применяется политика SDN.

> Сведения о подключении конечных точек контейнера к существующей клиентской виртуальной сети со стеком Microsoft SDN см. в статье [Подключение контейнеров к виртуальной сети](https://technet.microsoft.com/en-us/windows-server-docs/networking/sdn/manage/connect-container-endpoints-to-a-tenant-virtual-network).

> В Windows 10 Creators Update (и более поздних версиях) была реализована поддержка платформы, чтобы к работающему контейнеру можно было добавлять новую конечную точку контейнера (так называемое "горячее добавление").
...
### <a name="networking-modes--docker-drivers"></a>Сетевые режимы/драйверы Docker

  | Сетевой драйвер Windows для Docker | Типовые варианты использования | Контейнер-контейнер (один узел) | Контейнер-внешняя сеть (один узел + несколько узлов) | Контейнер-контейнер (несколько узлов) |
  |-------------------------------|:------------:|:------------------------------------:|:------------------------------------------------:|:-----------------------------------:|
  | **NAT (по умолчанию)** | Удобно для разработчиков | <ul><li>Одна подсеть: подключение с мостом через виртуальный коммутатор Hyper-V</li><li> Подключение между подсетями: не поддерживается в WS2016 (только один внутренний префикс NAT)</li></ul> | Маршрутизация через виртуальный сетевой адаптер управления (привязка к WinNAT) | Не поддерживается напрямую: требуется предоставление доступа к портам через узел |
  ...
  | **Наложение** | Требуется для Docker Swarm с несколькими узлами | <ul><li>Одна подсеть: подключение с мостом через виртуальный коммутатор Hyper-V</li><li>Подключение между подсетями: сетевой трафик инкапсулируется и маршрутизируется через виртуальный сетевой адаптер управления</li></ul> | Не поддерживается напрямую: требуется вторая конечная точка контейнера, подключенная к сети NAT | Одна подсеть/подключение между подсетями: сетевой трафик инкапсулируется с помощью VXLAN и маршрутизируется через виртуальный сетевой адаптер управления |
  | **L2Bridge** | Используется для Kubernetes и Microsoft SDN | <ul><li>Одна подсеть: подключение с мостом через виртуальный коммутатор Hyper-V</li><li> Подключение между подсетями: MAC-адрес контейнера повторно записывается при входе и выходе и маршрутизируется</li></ul> | MAC-адрес контейнера повторно записывается при входе и выходе | <ul><li>Одна подсеть: подключение с мостом</li><li>Подключение между подсетями: не поддерживается в WS2016.</li></ul> |
...
| -------------------------|:----:|
| NAT | Динамическое выделение и назначение IP-адресов сетевой службой узлов (HNS) из внутреннего префикса сети NAT |
...
| L2Bridge | Статическое выделение и назначение IP-адресов из числа адресов в префиксе сети узла контейнеров (также возможно назначение через подключаемый модуль службы HNS) |
...
| :---: | :---------------     |  :---                |
| nat | ДА | Н/Д |  
| overlay | ДА | ДА, с помощью Docker EE |
| прозрачный режим | НЕТ | НЕТ |
