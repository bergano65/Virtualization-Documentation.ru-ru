...
**Преобразование сетевых адресов** — этот сетевой режим удобен для быстрого назначения частных IP-адресов контейнерам. Внешний доступ к контейнеру предоставляется через сопоставление порта между внешним IP-адресом и портом (узел контейнера) и внутренним IP-адресом и портом контейнера. Весь сетевой трафик, полученный для сочетания внешнего IP-адреса и порта, сравнивается с таблицей сопоставления портов WinNAT и перенаправляется на IP-адрес и порт подходящего контейнера. Кроме того, NAT позволяет нескольким контейнерам размещать приложения, которым требуются одинаковые (внутренние) порты связи, сопоставляя их с уникальными внешними портами. Windows поддерживает только один внутренний префикс сети NAT для каждого узла. Дополнительные сведения см. в записи блога, посвященной [возможностям и ограничениям WinNAT](https://blogs.technet.microsoft.com/virtualization/2016/05/25/windows-nat-winnat-capabilities-and-limitations/). 

> Начиная с TP5, для всех сопоставлений статических портов NAT автоматически создается правило брандмауэра. Оно является глобальным по отношению к узлу контейнера и не ограничивается конкретным контейнером конечной точки или сетевым адаптером.

#### Конфигурация узла <!--1-->

Для использования сетевого режима NAT создайте сеть контейнера с именем драйвера "nat". 

> Так как для каждого узла можно создать только одну сеть _nat_ по умолчанию, убедитесь, что сеть NAT создается только после удаления всех остальных сетей NAT и запуска управляющей программы Docker с параметром -b "none". Если необходимо просто контролировать, какую внутреннюю IP-сеть использует сеть NAT, можно добавить параметр _--fixed-cidr=<внутренний префикс или маска NAT>_ в команду dockerd в C:\ProgramData\docker\runDockerDaemon.cmd.
...
> Чтобы создать несколько прозрачных сетей или сетей l2bridge, необходимо указать, к какому сетевому адаптеру (виртуальному) должен быть привязан внешний виртуальный коммутатор Hyper-V (создаваемый автоматически).
...
При создании сети контейнеров или самого контейнера можно указать различные сетевые параметры Docker. Помимо параметра -d (--driver=<network mode>), используемого для указания сетевого режима, при создании сети контейнера также поддерживаются параметры --gateway, --subnet и -o.
...
Статические IP-адреса задаются на сетевом адаптере контейнеров и поддерживаются только для сетевых режимов NAT, прозрачного взаимодействия (в ожидании [PR](https://github.com/docker/docker/pull/22208)) и L2Bridge. Кроме того, для сети "nat" по умолчанию не поддерживается назначение статических IP-адресов через Docker.
