...
---
# Сетевые подключения контейнеров Windows
...
# Сведения о сетевых подключениях контейнеров Windows

## Изоляция (пространство имен) с секциями сети
...
## Безопасность брандмауэра Windows
...
Расширение виртуальной платформы фильтрации (VFP)— это виртуальный коммутатор Hyper-V. Это расширение переадресации используется для применения сетевых политик и управления пакетами. Например, расширение VFP используется сетевым драйвером "overlay" для выполнения инкапсуляции VXLAN и драйвером "l2bridge" для перезаписи MAC-адреса во входящем и исходящем трафике. Расширение VFP присутствует только в Windows Server 2016 и обновлении Windows 10 Creators Update. Для проверки корректности работы расширения доступно две пользовательских команды.
...
#### Виртуальные машины Moby Linux используют коммутатор DockerNAT с Docker для Windows (продукт [Docker CE](https://www.docker.com/community-edition)) вместо внутреннего виртуального коммутатора службы HNS. 
