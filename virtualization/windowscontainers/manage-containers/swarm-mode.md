...
>[!NOTE]
>Любой заданной группы мелких объектов может быть несколько управляющих узлов, но должен быть *по крайней мере одну*. 
...
По крайней мере один физический или виртуальный компьютер системы (использовать все возможности режима мелких объектов по крайней мере два узла рекомендуется) под управлением **Windows 10 Creators Update** или **Windows Server 2016** *со всеми последние updates\ **, программой установки как узел контейнера (см. раздел, [контейнеры Windows в Windows 10](https://docs.microsoft.com/en-us/virtualization/windowscontainers/quick-start/quick-start-windows-10) или [контейнеры Windows в Windows Server](https://docs.microsoft.com/en-us/virtualization/windowscontainers/quick-start/quick-start-windows-server) для Дополнительные сведения о том, как приступить к работе с контейнерами Docker в Windows 10).
...
Несколько узлов, что *не* требуется использовать режим мелких объектов и режим сети наложения функции. Все функции группы мелких объектов и сети наложения доступны при наличии одного узла, работающего в режиме мелких объектов (т. е. управляющего узла, переведенного в режим мелких объектов с помощью команды `docker swarm init`).
...
>[!NOTE]
>[Метки объектов docker](https://docs.docker.com/engine/userguide/labels-custom-metadata/) можно использовать для применения метаданных к различным объектам Docker (включая образы контейнеров, контейнеры, тома и сетям), а для различных целей (например метки могут быть использованы для разделения «переднего плана» и «серверное» компоненты приложения, давая переднего плана микрослужб быть secheduled только на «переднего плана» меткой узлы и серверное mircoservices можно запланировать только на «серверное» меткой узлов). В этом случае метки используются для того, чтобы узлы ОС Windows можно было отличить от узлов ОС Linux.
