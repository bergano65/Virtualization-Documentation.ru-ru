...
Обновления годовщины для Windows Server 2016 и Windows 10 (обе версии 14393) были первыми выпусками Windows, которые могут создавать и запускать контейнеры Windows Server. Контейнеры, созданные с помощью этих версий, могут работать в более новых выпусках, но перед началом вы должны знать некоторые моменты.
...
## <a name="windows-server-host-os-compatibility"></a>Совместимость операционной системы Windows Server Host

<!-- start tab view -->
# [<a name="windows-server-version-1909"></a>Windows Server версии 1909](#tab/windows-server-1909)
...
# [<a name="windows-server-version-1903"></a>Windows Server версии 1903](#tab/windows-server-1903)
...
# [<a name="windows-server-2019"></a>WindowsServer2019](#tab/windows-server-2019)
...
# [<a name="windows-server-2016"></a>WindowsServer2016](#tab/windows-server-2016)
...
## <a name="windows-10-host-os-compatibility"></a>Совместимость операционной системы Windows 10 Host
...
Поскольку контейнеры Windows Server и нижележащий узел имеют один и тот же ядро, версия операционной системы для этого контейнера должна совпадать с версией основного образа. Если версии различаются, контейнер может начинаться, но полная функция не гарантирована. В операционной системе Windows есть четыре уровня управления версиями: основная, младшая, сборка и редакция. Например, версия 10.0.14393.103 имеет старшую версию 10, младшую версию 0, номер сборки 14393 и номер редакции 103. Номер сборки изменяется только при публикации новых версий ОС, например версии 1709, 1903 и т. д. Номер редакции меняется при установке обновлений Windows.
...
Способ 2: запросите следующий раздел реестра: HKEY_LOCAL_MACHINE \Софтваре\микрософт\виндовс Нт\куррентверсион
