...
<!-- start tab view -->
# <a name="windows-server-version-1909"></a>[Windows Server, версия 1909](#tab/windows-server-1909)
...
# <a name="windows-server-version-1903"></a>[Windows Server, версия 1903](#tab/windows-server-1903)
...
# <a name="windows-10-version-1909"></a>[Windows 10, версия 1909](#tab/windows-10-1909)
...
# <a name="windows-10-version-1903"></a>[Windows 10, версия 1903](#tab/windows-10-1903)
...
# <a name="windows-10-version-1809"></a>[Windows 10, версия 1809](#tab/windows-10-1809)
...
Контейнеры на базе Windows Server 2016 блокируются, когда номера редакции узла контейнера и образа контейнера отличаются. Например, если узел контейнера имеет версию 10.0.14393. **1914** (применяется Windows Server 2016 с KB4051033), а образ контейнера имеет версию 10.0.14393. **1944** (в Windows Server 2016 с KB4053579), образ не будет запущен.

Однако для узлов или образов, использующих Windows Server версии 1809 и выше, это правило не применяется, и образ узла и контейнера не должен иметь соответствующих редакций. 

Мы рекомендуем обеспечить актуальность систем (ведущих и контейнеров) с последними исправлениями и обновлениями, чтобы обеспечить безопасность.
...
Пример 2. узел контейнера работает под управлением Windows Server версии 1809 с примененным KB4534273. Все контейнеры Windows Server, развернутые на этом узле, должны быть основаны на базовом образе контейнера Windows Server версии 1809 (10.0.17763), но не должны совпадать с базой знаний узла. Если к узлу применяется KB4534273, образы контейнеров по-прежнему будут поддерживаться, но рекомендуется обновить их для решения любых потенциальных проблем безопасности.
...
1. Изменить `C:\ProgramData\docker\config\daemon.json`
