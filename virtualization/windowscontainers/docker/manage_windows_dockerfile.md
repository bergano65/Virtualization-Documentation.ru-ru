...
Этот пример устанавливает распространяемый пакет Visual Studio.
```none
RUN powershell.exe -Command c:\vcredist_x86.exe /quiet
``` 

В этом примере устанавливается пакет для разработчиков .NET Framework 4.5.2 Developer Pack. Для этого сначала извлекаются установочные файлы и затем запускается сам установщик. 
```none
