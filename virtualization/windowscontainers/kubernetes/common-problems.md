...
В конфигурации (по умолчанию) Кубе-прокси для кластеров с 100 + балансировщик нагрузки могут работать недоступные временные TCP-порты (они динамический диапазон портов, который обычно охватывает порты 49152 – 65535) из-за большого количества портов, зарезервированных на каждом узле для каждой подсистемы балансировки нагрузки (не DSR). Это может проявляться с ошибками в Кубе-прокси, например:
```
